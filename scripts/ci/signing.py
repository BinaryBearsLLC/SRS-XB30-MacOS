...
def main():
    work = Path(os.environ['RUNNER_TEMP']) / 'xb30-signing'
    ...
    try:
        ...
        run('xcrun', 'notarytool', 'store-credentials', 'xb30-release', '--key', str(api_key),
    ...
    with open(os.environ['GITHUB_ENV'], 'a') as env:
        for key, value in {'MACOS_SIGN_IDENTITY': identity, 'SIGNING_KEYCHAIN': keychain,
                           'NOTARY_PROFILE': 'xb30-release', 'SIGNED_MACOS': '1'}.items():
