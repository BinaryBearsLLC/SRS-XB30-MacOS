...
export default function Speaker({state,bassLevel}:{state:SpeakerState;bassLevel:()=>number}){
 ...
 useEffect(()=>{
  ...
  },undefined,()=>{if(alive)setLoad('error')});
  const resize=()=>{const {width,height}=el.getBoundingClientRect();if(!width||!height)return;renderer.setSize(width,height);composer.setSize(width,height);camera.aspect=width/height;camera.updateProjectionMatrix()};const ro=new ResizeObserver(resize);ro.observe(el);resize();const reduced=matchMedia('(prefers-reduced-motion: reduce)');
  function draw(ms:number){if(!alive)return;const s=live.current,animate=s.motion&&!reduced.matches,energy=s.playing?audioLevel.current():0,f=lightFrame(s.light,ms/1000,animate,s.brightness,s.strip,s.strobes,energy);
   for(const m of strips){const pos=m.geometry.attributes.position,colors=m.geometry.attributes.color;
    for(let i=0;i<pos.count;i++){const phase=(((f.spatial?Math.atan2(pos.getY(i)/.31,pos.getX(i)/1.035)/Math.PI/2:0)+1+f.phase)%1)*f.palette.length;const j=Math.floor(phase);a.set(f.palette[j%f.palette.length]);b.set(f.palette[(j+1)%f.palette.length]);color.copy(a).lerp(b,phase-j).multiplyScalar(f.intensity);if(!f.intensity)color.setRGB(.05,.07,.08);colors.setXYZ(i,color.r,color.g,color.b)}colors.needsUpdate=true;
   }
